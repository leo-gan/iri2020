...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="IRI2020 surrogate short training + benchmark")
    ...
    from iri2020.surrogate.data import (
        generate_samples, concat_batches, save_batch, load_batch, train_val_test_split,
        ensure_iri_data_dir,
    )
    ...
    if not args.skip_generate and not train_p.exists():
        ...
        def take(b, ii):
            return IRISampleBatch(
                doy=b.doy[ii], hour=b.hour[ii], year=b.year[ii], month=b.month[ii], day=b.day[ii],
                glat=b.glat[ii], glon=b.glon[ii], alt_km=b.alt_km[ii], f107=b.f107[ii], ap=b.ap[ii],
                y=b.y[ii], targets=b.targets, regime=b.regime[ii],
            )

        full_nom = nom
        tr, va, te = train_val_test_split(full_nom, args.n_val, args.n_test, seed=args.seed)
        if n_ext_tr > 0:
        ...
        print(f"saved batches: train={len(tr.doy)} val={len(va.doy)} test={len(te.doy)} extreme={len(ext_eval.doy)}", flush=True)
    else:
    ...
    if not args.skip_benchmark:
        ...
        run_benchmark(art, test_batch=te, extreme_batch=ext_eval, config=cfg, n_speed_samples=24)
